...
class WowModel():
    def __init__(self,LH_NAME,RH_NAME,LF_NAME,RF_NAME,q_ID_marker):
        self.urdf_filename = "../../../resources/robots/g1/urdf/g1.urdf"
    ...
    def ik(self,q,T_LH,T_RH,T_LF,T_RF,R_LH,R_RH,R_LF,R_RF):
        ...
        while True:
            ...
            if norm(err_lh) > self.eps_hand:
                J1 = pin.computeFrameJacobian(self.model, self.data, q, self.LH_ID) # in joint frame
                J1 = -np.dot(pin.Jlog6(fMd_lh.inverse()), J1)
                v1 = -J1.T.dot(solve(J1.dot(J1.T) + self.damp * np.eye(6), err_lh))
                q1 = pin.integrate(self.model, q, v1 * self.DT)
            ...
            if norm(err_rh) > self.eps_hand:
                J2 = pin.computeFrameJacobian(self.model, self.data, q, self.RH_ID) # in joint frame
                J2 = -np.dot(pin.Jlog6(fMd_rh.inverse()), J2)
                v2 = -J2.T.dot(solve(J2.dot(J2.T) + self.damp * np.eye(6), err_rh))
                q2 = pin.integrate(self.model, q, v2 * self.DT)
            ...
            if norm(err_lf) > self.eps_foot:
                J3 = pin.computeFrameJacobian(self.model, self.data, q, self.LF_ID) # in joint frame
                J3 = -np.dot(pin.Jlog6(fMd_lf.inverse()), J3)
                v3 = -J3.T.dot(solve(J3.dot(J3.T) + self.damp * np.eye(6), err_lf))
                q3 = pin.integrate(self.model, q, v3 * self.DT)
            ...
            if norm(err_rf) > self.eps_foot:
                J4 = pin.computeFrameJacobian(self.model, self.data, q, self.RF_ID) # in joint frame
                J4 = -np.dot(pin.Jlog6(fMd_rf.inverse()), J4)
                v4 = -J4.T.dot(solve(J4.dot(J4.T) + self.damp * np.eye(6), err_rf))
                q4 = pin.integrate(self.model, q, v4 * self.DT)
    ...
    def print_results(self):
        ...
        print("q: %s" % self.q.T)
        print("\nJoint placements:")
        for name, oMf in zip(self.model.names, self.data.oMf):
            if name==self.LH_NAME or name==self.RH_NAME or name==self.LF_NAME or name==self.RF_NAME:
                print(("{:<24} : {: .4f} {: .4f} {: .4f}".format(name, *oMf.translation.T.flat)))
        print("\nJoint rotation:")
        for name, oMf in zip(self.model.names, self.data.oMf):
            if name==self.LH_NAME or name==self.RH_NAME or name==self.LF_NAME or name==self.RF_NAME:
                print(("{:<24} : {: .4f} {: .4f} {: .4f} {: .4f} {: .4f} {: .4f} {: .4f} {: .4f} {: .4f}".format(name, *oMf.rotation.flat))) 
